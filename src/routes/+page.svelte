<script lang="ts">
	...
	let cardStats = { total: 0, interesting: 0, not_interesting: 0, unrated: 0 };
	let viewMode = 'random' as 'random' | 'interesting' | 'not_interesting' | 'manage';
	...
	async function checkLocalDatabase() {
		try {
			...
			if (data.success) {
				...
				cardStats = data.stats || { total: 0, interesting: 0, not_interesting: 0, unrated: 0 };
				...
				localCardCount = 0;
				cardStats = { total: 0, interesting: 0, not_interesting: 0, unrated: 0 };
			}
		} catch (err) {
	...
	async function fetchRandomCard() {
		...
		try {
			...
			if (err instanceof Error && err.message.includes('No commander cards found')) {
				error = 'No commander cards found in database. Please update the database first.';
			} else {
	...
	async function refillCardQueue() {
		...
		try {
			...
			const preloadPromises = Array(cardsToLoad).fill(null).map(() => preloadCard());
			const preloadedCards = await Promise.all(preloadPromises);
			
			// Add successfully loaded cards to queue, filter out nulls
			const validCards = preloadedCards.filter((card): card is Card => card !== null);
			cardQueue.push(...validCards);
			
			console.log(`Preloaded ${validCards.length} cards. Queue size: ${cardQueue.length}`);
		} finally {
			preloading = false;
		}
	}
	...
	async function fetchCardsByRating(rating: 'interesting' | 'not_interesting'): Promise<Card[]> {
		try {
			const response = await fetch(`/api/cards/by-rating?rating=${rating}`);
			const data = await response.json();
	...
	async function rateCard(targetRating: 'interesting' | 'not_interesting' | null) {
		if (!card?.id) {
		...
		try {
			...
			if (data.success) {
				...
				if (card) {
					card.interest_rating = targetRating;
				}
				// Refresh stats
				await checkLocalDatabase();
				
				// Auto-load next card for interesting/not_interesting ratings
				if (targetRating === 'interesting' || targetRating === 'not_interesting') {
	...
	async function exportRatings() {
		...
		try {
			...
			if (data.success) {
				...
				const blob = new Blob([JSON.stringify(data.ratings, null, 2)], { type: 'application/json' });
		...
		try {
			...
			input.onchange = async (e) => {
				...
				try {
					...
					importError = 'Failed to parse or import file. Please check the file format.';
	...
	async function clearProgress() {
		...
		try {
			...
			if (data.success) {
				...
				cardStats = { total: cardStats.total, interesting: 0, not_interesting: 0, unrated: cardStats.total };
				interestingCards = [];
				notInterestingCards = [];
				// Clear current card rating if it exists
				if (card) {
					card.interest_rating = null;
				}
			} else {
				clearError = data.message;
			}
	...
	function getCardData(card: Card, faceIndex: number = 0) {
		if (isDoubleFaced(card) && card.card_faces && card.card_faces[faceIndex]) {
			const face = card.card_faces[faceIndex];
			return {
				name: face.name,
				oracle_text: face.oracle_text,
				image_uri_png: face.image_uri_png,
				colors: face.colors,
				power: face.power,
...
<div class="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 py-8">
    ...
    <div class="container mx-auto px-4">
		...
		{#if !databaseInitialized}
			<div class="max-w-2xl mx-auto mb-8">
				<div class="bg-blue-500/20 backdrop-blur-sm rounded-lg p-6 shadow-lg text-center">
					<h2 class="text-2xl font-bold text-white mb-4">Setting up your commander database...</h2>
					<div class="flex justify-center mb-4">
						<div class="animate-spin rounded-full h-8 w-8 border-b-2 border-white"></div>
					</div>
					<p class="text-gray-300 text-sm">
						{initializing ? 'Initializing database...' : 'Loading...'}
					</p>
				</div>
			</div>
		{/if}
	...
	{#if databaseInitialized}
		<div class="max-w-7xl mx-auto flex gap-6">
			...
			<div>
				<div class="bg-white/10 backdrop-blur-sm rounded-lg p-6 shadow-lg sticky top-4">
					{#if localCardCount > 0}
						<h3 class="font-semibold text-white mb-4 text-center">Database Stats</h3>
						<div class="space-y-3 mb-4">
							<div class="bg-blue-500/20 rounded-lg p-2 text-center">
								<p class="text-xl font-bold text-blue-300">{cardStats.total.toLocaleString()}</p>
								<p class="text-xs text-gray-300">Total</p>
							</div>
							<div class="bg-green-500/20 rounded-lg p-2 text-center">
								<p class="text-xl font-bold text-green-300">{cardStats.interesting.toLocaleString()}</p>
								<p class="text-xs text-gray-300">Interesting</p>
							</div>
							<div class="bg-red-500/20 rounded-lg p-2 text-center">
								<p class="text-xl font-bold text-red-300">{cardStats.not_interesting.toLocaleString()}</p>
								<p class="text-xs text-gray-300">Not Interesting</p>
							</div>
							<div class="bg-gray-500/20 rounded-lg p-2 text-center">
								<p class="text-xl font-bold text-gray-300">{cardStats.unrated.toLocaleString()}</p>
						...
						<div class="mt-4 text-center">
							<p class="text-xs text-gray-400">
								📦 Queue: {cardQueue.length}/{QUEUE_TARGET_SIZE}
								{#if preloading}
					...
					{:else}
						<h3 class="font-semibold text-white mb-4 text-center">Database Setup</h3>
						<div class="bg-yellow-500/20 rounded-lg p-4 mb-4 text-center">
							<p class="text-yellow-200 text-sm">Database is empty</p>
							<p class="text-yellow-300 text-xs mt-1">Click "Sync Cards" below to populate</p>
						</div>
					{/if}
				</div>
			</div>
		...
		<div class="max-w-6xl mx-auto">
			...
			{#if viewMode == 'random'}
				<div class="bg-white/5 backdrop-blur-sm rounded-lg p-6">
					<RandomCardDisplay
						...
						on:rateCard={(e: CustomEvent<{rating: string}>) => rateCard(e.detail.rating as 'interesting' | 'not_interesting')}
			...
			<div class="bg-white/5 backdrop-blur-sm rounded-lg p-6">
				<div class="max-w-2xl mx-auto space-y-6">
					<h2 class="text-2xl font-bold text-white text-center mb-6">Database Management</h2>
					
					<!-- Sync Database Section -->
					<div class="bg-white/10 rounded-lg p-4">
						<h3 class="text-lg font-semibold text-white mb-3">🔄 Sync Database</h3>
						...
						<!-- Sync Results -->
						{#if syncResult}
							<div class="bg-green-500/20 border border-green-400 text-green-100 px-3 py-2 rounded text-sm mt-3">
								<p class="font-semibold">✅ Sync Success</p>
								<p>{syncResult}</p>
							</div>
						{/if}
						
						{#if syncError}
							<div class="bg-red-500/20 border border-red-400 text-red-100 px-3 py-2 rounded text-sm mt-3">
								<p class="font-semibold">❌ Sync Error</p>
								<p>{syncError}</p>
							</div>
						{/if}
						
						{#if syncing}
							<div class="bg-blue-500/20 border border-blue-400 text-blue-100 px-3 py-2 rounded text-sm mt-3">
								<p class="font-semibold">🔄 Syncing in Progress</p>
								<p>Processed: {syncProgress.processed.toLocaleString()} / {syncProgress.total.toLocaleString()}</p>
								{#if syncProgress.currentCard}
									<p class="text-xs opacity-75">Current: {syncProgress.currentCard}</p>
								{/if}
							</div>
						{/if}
					</div>
					...
					<div class="bg-white/10 rounded-lg p-4">
						<h3 class="text-lg font-semibold text-white mb-3">📂 Import/Export Ratings</h3>
						<p class="text-gray-300 text-sm mb-4">Backup your ratings or import from another database</p>
						...
						{#if exportResult}
							<div class="bg-green-500/20 border border-green-400 text-green-100 px-3 py-2 rounded text-sm mt-3">
								<p class="font-semibold">✅ Export Success</p>
								<p>{exportResult}</p>
							</div>
						{/if}
						...
						{#if importResult}
							<div class="bg-green-500/20 border border-green-400 text-green-100 px-3 py-2 rounded text-sm mt-3">
								<p class="font-semibold">✅ Import Success</p>
								<p>{importResult}</p>
							</div>
						{/if}
					...
					<div class="bg-red-500/10 border border-red-400/30 rounded-lg p-4">
						<h3 class="text-lg font-semibold text-red-300 mb-3">⚠️ Danger Zone</h3>
						<p class="text-gray-300 text-sm mb-4">These actions cannot be undone</p>
						...
						<div class="grid grid-cols-2 gap-3 mt-2 text-xs text-gray-400">
...
{/if} <!-- End of databaseInitialized conditional -->
